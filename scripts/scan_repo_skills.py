...
Usage:
    ...
    python scan_repo_skills.py --catalog           # scans ./publish by default
    python scan_repo_skills.py --catalog ./publish --output publish_skills.html
...
# ---------------------------------------------------------------------------
# Scan: catalog directory  (e.g. publish/)
# ---------------------------------------------------------------------------

_CATALOG_TABLE_ROW_RE = re.compile(
    r"^\|\s*\*\*(.+?)\*\*\s*\|\s*(.*?)\s*\|", re.MULTILINE
)


def _parse_catalog_entry(content: str) -> tuple[str | None, str, dict]:
    """Parse a catalog .md file (generated by sync_to_local.py) into (name, description, metadata)."""
    name: str | None = None
    description = ""
    metadata: dict = {}
    ...
    # Name from the first H1 heading (single #)
    for line in lines:
        stripped = line.strip()
    ...
    # Metadata from | **Field** | Value | table rows — skip the header row
    for m in _CATALOG_TABLE_ROW_RE.finditer(content):
        key = m.group(1).strip()
...
def scan_catalog(catalog_dir: str, max_file_bytes: int) -> dict:
    """Scan a catalog directory (e.g. publish/) for .md skill entries."""
    root_path = pathlib.Path(catalog_dir).resolve()
    if not root_path.is_dir():
        raise ScanError(f"Not a directory: {catalog_dir}")
    ...
    for md_path in sorted(root_path.glob("*.md")):
        ...
        name, description, metadata = _parse_catalog_entry(content)

        # Skip files with no metadata table or no description (e.g. SKILLS-GUIDE.md, README.md)
        if not name or not metadata or not description:
            continue
        ...
        if len(content_bytes) > max_file_bytes:
            ...
            file_entry["b64"] = base64.b64encode(content_bytes).decode("ascii")
...
def render_html(data: dict) -> str:
    ...
    if data.get("source") in ("local", "catalog"):
...
/* ---- wire up ------------------------------------------------------------ */
if (DATA.source === "local" || DATA.source === "catalog") {
  document.getElementById("title").textContent = "Skills in " + esc(DATA.label);
  const sourceLabel = DATA.source === "catalog" ? "catalog" : "local path";
  document.getElementById("subtitle").textContent =
    DATA.count + " skill(s) · " + sourceLabel + ": " + DATA.root;
} else {
...
def main(argv: list[str] | None = None) -> int:
    ...
    source.add_argument(
        "--path",
        default=".",
        help="Local directory to scan for SKILL.md files (default: current directory).",
    )
    source.add_argument(
        "--catalog",
        metavar="DIR",
        nargs="?",
        const="./publish",
        help="Scan a catalog directory for .md skill entries (default: ./publish).",
    )
    parser.add_argument(
    ...
    try:
        if args.repo:
            ...
            catalog_dir = args.catalog or "./publish"
            print(f"Scanning catalog {catalog_dir} …", file=sys.stderr)
            data = scan_catalog(catalog_dir, args.max_file_bytes)
            label = data["label"]
        else:
